...
const PIPELINE_EP = `You are the Elevation Nation Content Intelligence Engine for @everydayelevations. Analyze this research and return ONLY JSON: {"summary":"2-3 sentence synthesis","reelIdeas":[{"title":"hook-driven title","hook":"exact opening line","why":"why drives follows","cta":"specific CTA"}],"topGap":"single most important content gap in one sentence","elevationNationAngle":"how to leverage Elevation Nation community identity"}`;

const PIPELINE_SP = `You are a Reel script writer for @everydayelevations. VOICE: Conversational, no fluff, direct, short punchy sentences, Elevation Nation community woven in. TARGET: 30-60 seconds. Return ONLY JSON: {"hook":"first line","hookNote":"delivery direction","body":["l1","l2","l3","l4","l5"],"bodyNotes":["n1","n2","n3","n4","n5"],"cta":"closing line","ctaNote":"how to land it","onScreenText":"opening overlay","caption":"full Instagram caption 3-5 paragraphs","hashtags":["#t1","#t2","#t3","#t4","#t5","#t6","#t7"],"estimatedSeconds":45}`;
...
const ft = () => result?.scr ? [
`[ON SCREEN: ${result.scr.onScreenText}]`, `, `HOOK: "${result.scr.hook}"`, `(${result.scr.hookNote})`, `,
...
<style>{`@import url('https://fonts.googleapis.com/css2?family=Bebas+Neue&family=Barlow:wght@400;500;600;700&display=swap'); *{box-sizing:border-box;} input,textarea{outline:none;} input:focus,textarea:focus{border-color:${B.red}!important; box-shadow:0 0 0 3px ${B.red}22!important;} @keyframes fu{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}} @keyframes spin{to{transform:rotate(360deg)}} button{font-family:'Barlow',sans-serif;} ::-webkit-scrollbar{width:5px;} ::-webkit-scrollbar-track{background:${B.offWhite};} ::-webkit-scrollbar-thumb{background:${B.border};border-radius:3px;}`}</style>
