...
// ─── PROMPT VAULT TABS (unchanged logic) ─────────────────────────────────────
const VAULT_TABS = [
  {
    id: 'instagram',
    label: 'Instagram',
...
// ─── AI HELPERS ──────────────────────────────────────────────────────────────
async function ai(message: string, system = 'You are a helpful content strategist.') {
...
async function perp(query: string) {
  try {
...
const SecLabel: React.FC<{ children: React.ReactNode; style?: React.CSSProperties }> = ({ children, style }) => (
  <div
    style={{
      fontSize: 10,
      fontWeight: 700,
      ...
      {displayAngles.map((a: any) => (
        <button
          ...
          style={{
            ...
            border: `1px solid ${
              selected === a.id ? B.red : a.custom ? 'rgba(245,166,35,0.25)' : 'rgba(255,255,255,0.1)'
            }`,
            borderRadius: 8,
            padding: '10px 6px',
            cursor: 'pointer',
...
const StrategyOutput: React.FC<{
  text: string;
  onDownload: () => void;
  downloading: boolean;
}> = ({ text, onDownload, downloading }) => {
  if (!text) return null;
  ...
  const renderInline = (inlineText: string) => {
    ...
    return parts.map((p, i) =>
      p.startsWith('**') && p.endsWith('**') ? (
        <strong key={i} style={{ color: B.white, fontWeight: 700 }}>
          {p.replace(/\*\*/g, '')}
        </strong>
      ) : (
        p
  ...
  const renderLine = (line: string, idx: number) => {
    ...
    if (line === '---' || line === '***')
      return <div key={idx} style={{ borderTop: '1px solid rgba(255,255,255,0.08)', margin: '16px 0' }} />;
    ...
    try {
      const agencyName = (() => {
        try {
          ...
          return 'SIGNAL by Everyday Elevations';
        }
      })();
      const accentColor = (() => {
        try {
      ...
      const styledLines = text.split('\n').map((line) => {
        ...
        if (line.startsWith('## ')) {
          sectionNum++;
          return (
            '<div class="section-header"><span class="section-num">' +
            sectionNum +
            '</span><span class="section-title">' +
            escHtml(line.slice(3)) +
            '</span></div>'
          );
        }
        if (line.startsWith('### ')) {
          return (
            '<div class="subsection">' + boldify(escHtml(line.slice(4))) + '</div>'
          );
        }
        if (line.startsWith('#### ')) {
          return (
            '<div class="subsubsection">' +
            boldify(escHtml(line.slice(5))) +
            '</div>'
          );
        }
        if (
        ...
        if (line.startsWith('- ') || line.startsWith('* ')) {
          return (
            '<div class="list-item"><span class="bullet">&#8226;</span><span>' +
            boldify(escHtml(line.slice(2))) +
            '</span></div>'
          );
        }
        if (/^\d+\.\s/.test(line)) {
          ...
          return (
            '<div class="list-item"><span class="bullet">' +
            num +
            '.</span><span>' +
            boldify(escHtml(line.replace(/^\d+\.\s/, ''))) +
            '</span></div>'
          );
        }
      ...
      const css = [
        ...
        '.cover-agency { font-size: 10px; font-weight: 700; letter-spacing: 3px; text-transform: uppercase; color: ' +
          accentColor +
          '; margin-bottom: 32px; }',
        ...
        '.cover-meta-label { font-size: 9px; color: ' +
          accentColor +
          '; font-weight: 700; letter-spacing: 2px; text-transform: uppercase; margin-bottom: 4px; }',
        '.cover-meta-value { font-size: 12px; color: rgba(255,255,255,0.8); font-weight: 500; line-height: 1.4; }',
        '.content { padding: 32px 40px; }',
        '.doc-title { font-size: 22px; font-weight: 900; color: #080D14; letter-spacing: -0.03em; margin: 32px 0 8px; padding-bottom: 12px; border-bottom: 2px solid ' +
          accentColor +
          '; }',
        ...
        '.section-num { background: ' +
          accentColor +
          '; color: #000; font-size: 11px; font-weight: 900; width: 22px; height: 22px; border-radius: 3px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }',
        '.section-title { font-size: 12px; font-weight: 800; letter-spacing: 1.5px; text-transform: uppercase; }',
        '.subsection { font-size: 13px; font-weight: 700; color: ' +
          accentColor +
          '; margin: 20px 0 6px; letter-spacing: 0.02em; }',
        '.subsubsection { font-size: 12px; font-weight: 700; color: #333; margin: 14px 0 4px; }',
        '.label { font-size: 11px; font-weight: 700; color: #080D14; background: #f5f5f5; padding: 4px 10px; border-left: 3px solid ' +
          accentColor +
          '; margin: 10px 0 4px; }',
        ...
        '.list-item .bullet { color: ' +
          accentColor +
          '; font-weight: 700; flex-shrink: 0; margin-top: 1px; min-width: 16px; }',
      ...
      const coverHtml =
        ...
        escHtml(title.split(' — ')[0] || title) +
        '</div>' +
        '<div class="cover-subtitle">Social Media Strategy Document</div>' +
        '<div class="cover-meta">' +
        '<div class="cover-meta-item"><div class="cover-meta-label">Prepared By</div><div class="cover-meta-value">' +
        agencyName +
        '</div></div>' +
        '<div class="cover-meta-item"><div class="cover-meta-label">Document Date</div><div class="cover-meta-value">' +
        today +
        '</div></div>' +
      ...
      const footerHtml =
        ...
        agencyName +
        ' &#8212; Confidential | Internal Use Only</div><div>Generated ' +
        today +
        '</div></div>';
...
// ─── (All your prompt builders, ContentMemory, Strategy Builder, etc.) ───────
// The rest of the file from your paste is already valid React/JSX and can
// remain as-is under this import/React setup.[file:16]

// For brevity I’m omitting repetition here; in your repo, keep all the
// remaining components (ContentMemory, StrategyBuilder, RepurposeEngine,
// HookLibrary, DesignStudio, Analytics, etc.) exactly as they are, just
// making sure they are below the imports and that React is imported.
// [file:16]

// ─── ROOT APP COMPONENT ──────────────────────────────────────────────────────
function App() {
  const [tab, setTab] = useState<'strategy' | 'scripts' | 'memory' | 'vault'>('strategy');

  return (
    <>
      <Head>
        <title>Everyday Elevations — Content Intelligence Studio</title>
        <meta name="description" content="Content strategy and memory for Everyday Elevations" />
      </Head>
      <div
        style={{
          minHeight: '100vh',
      ...
      >
        {/* Top shell, tabs, etc. – keep your existing layout here */}
        {/* Example minimal tab header: */}
        <div
          style={{
            maxWidth: 1200,
            margin: '0 auto',
        ...
        >
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div>
                <h1 style={{ margin: 0, fontSize: 20 }}>Everyday Elevations — Content Intelligence Studio</h1>
                <p style={{ margin: '4px 0 0', fontSize: 12, color: B.light }}>
          ...
          {/* Here render your big feature components according to tab,
             using the implementations from your paste (unchanged) */}
          {/* e.g. <StrategyBuilder />, <ScriptLab />, <ContentMemory />, <PromptVault /> */}
        </div>
      </div>
    </>
  );
...
// Optional: Error boundary if you still need it.
